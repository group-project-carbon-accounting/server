...
async def process_payment_with_footprint(account_id, amount, vendor, footprint, confidence):
    try:
        ...
        except general_handler_error.NoAssociatedFootprintAccountError:
            await cash_transaction_id
        ...
        try:
            await general_handlers.add_footprint_transaction(footprint_account_id, footprint, timestamp_, cash_transaction_id, 0, confidence)
        except general_handler_error.GeneralHandlerError as e:
            return str(e)
        finally:
            return cash_transaction_id